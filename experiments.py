...
from TRPCA.data import GetData
from TRPCA.method import Model
from TRPCA.getGipKernel import getGipKernel
...
class Experiments(object):
    def __init__(self, mir_dis_data, model_name='trpca_tnn', **kwargs):
        super().__init__()
    ...
    def CV_type(self):
        ...
        for k in range(k_folds):
            ...
            dis_sim = np.array(0.9* self.mir_dis_data.dis_sim + 0.1 * getGipKernel(train_matrix.T))
            miRNA_func_similarity_matrix = np.array(0.5 * self.mir_dis_data.mir_fun_sim_matrix + 0.5* getGipKernel(train_matrix))

            predict_tensor = self.model()(train_tensor, dis_sim, miRNA_func_similarity_matrix, self.mir_dis_data.type_tensor,
                                             lambda_=self.parameters['lambda_'], mu=self.parameters['mu'],
    ...
    def CV_triplet(self):
        ...
        for k in range(k_folds):
            ...
            train_matrix = train_tensor.sum(2)
            train_matrix[np.where(train_matrix > 0)] = 1
            dis_sim = np.array(0.5 * self.mir_dis_data.dis_sim + 0.5* getGipKernel(train_matrix.T))
            miRNA_func_similarity_matrix = np.array(0.9 * self.mir_dis_data.mir_fun_sim_matrix + 0.1 * getGipKernel(train_matrix))
            predict_tensor = self.model()(train_tensor, dis_sim, miRNA_func_similarity_matrix, self.mir_dis_data.type_tensor,
                                          lambda_=self.parameters['lambda_'], mu=self.parameters['mu'],
                                          alpha=self.parameters['alpha'])
    ...
    def get_metrics(self, real_score, predict_score):
        ...
        negative_index = np.where(predict_score_matrix < thresholds.T)# np.fil
...
if __name__ == '__main__':
    ...
    experiment = Experiments(mir_dis_data, model_name='trpca_tnn',  lambda_ =t, mu = 1e-4, alpha=0.3)
  
    ans.append(experiment.CV_type())
    ## ans.append(experiment.CV_triplet())
    print(ans)
